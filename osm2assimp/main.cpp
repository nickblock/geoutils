...
int main(int argi, char **argc) {
  ...
  args::ArgumentParser parser(
      "osm2assimp, convert osm files to geometry to be saved in a assimp "
      "compatible format.",
      "You must at least specify an input and an output file");
  args::HelpFlag help(parser, "HELP", "Show this help menu.", {'h', "help"});
  args::ValueFlag<std::string> inputFileArg(
      parser, "*.osm|*.pbf",
      "Specify input .osm file or comma separated list of files.", {'i'});
  args::ValueFlag<std::string> outputFileArg(
      parser, assimpWriter.formatsAvailableStr(),
      "Specify output file. The extension will be used to decide the output "
      "type. The type should be compatible with assimp",
      {'o'});
  args::ValueFlag<float> fixedHeightArg(
      parser, "100.0",
      "Specify a default height to be used in absence of heights data", {'f'});
  args::ValueFlag<int> consolidateArg(
      parser, "Consolidate",
      "Consolidate mesh data into single mesh '0' or Mesh per Object '2'. "
      "Default is mesh per Material/Type '1'",
      {'c'});
  args::Flag highwayArg(parser, "Highways", "Include roads in export", {'r'});
  args::Flag exportZUpArg(parser, "Z Up", "Export Z up", {'z'});
  args::Flag groundArg(parser, "Ground",
                       "Create ground plane to extents or S2 cell region",
                       {'g'});
  args::Flag convertCEF(
      parser, "Convert CEF",
      "When converting LatLng to coordinate use Center Earth Fixed algorithm, "
      "as opposed to the default mercator projection",
      {'a'});
  args::ValueFlag<string> extentsArg(
      parser, "Extents",
      "4 comma separated values; min lat, min long, max lat, max long", {'e'});
  args::ValueFlag<string> refPointArg(
      parser, "RefPoint",
      "2 comma separated values; latLng coords. To be used as point of origin "
      "for geometry ",
      {'p'});
  args::ValueFlag<string> s2CellArg(parser, "S2Cell",
                                    "S2 Cell Id as hex string. Filters nodes "
                                    "and ways only somepart inside S2 Cell",
                                    {'s', "s2"});
  args::ValueFlag<float> uvScaleArg(
      parser, "UV Scale",
  ...
  if (consolidateArg) {
    ...
    assimpWriter.setMeshGranularity(
        static_cast<AssimpWriter::MeshGranularity>(choice));
  }
  ...
  // the originLocation will decide the point of origin for the exported
  // geometry file. if it's not specified by the refPoint cmd line arg it is
  // taken from the bottom left corner of the box, which in turn can be decided
  // by the extents argument,
  ...
  if (extentsArg) {
    try {
      ...
      cout << "Failed to parse extents string '" << args::get(extentsArg) << "'"
           << endl;
      std::exit(1);
    }
  }
  ...
  if (refPointArg) {
    try {
      ...
      cout << "failed to parse ref point string '" << args::get(refPointArg)
           << "'" << endl;
      std::exit(1);
    }
  }
  ...
  if (s2CellArg) {
    try {
      ...
      if (groundArg) {
        ...
        for (int i = 0; i < 4; i++) {
          osmium::geom::Coordinates coord = ConvertLatLngToCoords::to_coords(
              {std::get<1>(cornerCoords[i]), std::get<0>(cornerCoords[i])});
          groundCorners[i] = {coord.x, coord.y};
        }
      }
    } catch (const std::exception &) {

      cout << "failed to parse S2 Cell id string '" << args::get(s2CellArg)
           << "', is it a <=16 character hex string?" << endl;
      std::exit(1);
    }
  }
  ...
  for (auto &inputFile : inputFiles) {
    ...
    try {
      ...
      osmium::io::Reader osmFileReader{inputFile,
      ...
      if (!box.valid()) {
        ...
        if (!box.valid() && !refPointArg) {
          cout << "No bounds found in osm file and no RefPoint specified, "
                  "aborting"
               << endl;
          exit(1);
        }
      ...
      // if the refPoint was not set via commandline take the bottom left of the
      // first input file specified
      if (!originLocation) {
        originLocation = box.bottom_left();
        ConvertLatLngToCoords::setRefPoint(originLocation);
      }
  ...
  if (sceneConstruct.wayCount()) {
    ...
    if (AI_SUCCESS != sceneConstruct.write(outputFile, assimpWriter, config)) {
      cout << "Failed to write out to '" << outputFile << "', "
           << assimpWriter.exporterErrorStr() << endl;
      return 1;
    }
  } else {
