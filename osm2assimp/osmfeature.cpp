  ...
  for (const auto &taglist : NameTags) {
    ...
    for (const auto &tag : taglist) {
      if (way.tags().has_key(tag.c_str())) {
        name +=
            std::string(way.tags().get_value_by_key(tag.c_str(), "noname")) +
            std::string(" ");
      } else {
        foundAll = false;
        break;
...
OSMFeature::OSMFeature(const std::vector<glm::vec2> &points, float height,
                       int type, const std::string &name)
    : mWorldCoords(points), mType(type), mHeight(height), mName(name) {
...
OSMFeature::OSMFeature(const osmium::Way &way, bool getNameFromOSM)
    : mHeight(determineHeightFromWay(way)), mType(determineTypeFromWay(way)),
      mValid(true) {
  ...
  for (auto node : way.nodes()) {
    ...
    osmium::geom::Coordinates c =
        ConvertLatLngToCoords::to_coords(node.location());
  ...
  if ((mType & HIGHWAY && mWorldCoords.size() < 2) ||
