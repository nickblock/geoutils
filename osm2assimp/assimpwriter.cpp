...
void AssimpWriter::setMeshGranularity(MeshGranularity m) { mGran = m; }

AssimpWriter::NodeAndMesh
AssimpWriter::mergeMeshes(std::vector<aiMesh *> meshes, int meshIdx) {
  ...
  for (size_t m = 0; m < meshes.size(); m++) {
    ...
    memcpy(&mergedMesh->mVertices[curVertices], meshes[m]->mVertices,
           meshes[m]->mNumVertices * sizeof(aiVector3D));

    memcpy(&mergedMesh->mNormals[curVertices], meshes[m]->mNormals,
           meshes[m]->mNumVertices * sizeof(aiVector3D));
...
void AssimpWriter::buildMeshPerMaterial(aiScene *assimpScene) {
  ...
  for (int i = 0; i < mMeshes.size(); i++) {
    if (materialMeshMap.find(mMeshes[i]->mMaterialIndex) ==
        materialMeshMap.end()) {
      materialMeshMap[mMeshes[i]->mMaterialIndex] = {};
    }
    freezeMesh(mMeshes[i], mMeshParents[i]);
    materialMeshMap[mMeshes[i]->mMaterialIndex].push_back(mMeshes[i]);
  }
  ...
  for (auto meshIter : materialMeshMap) {
    std::tie(assimpScene->mMeshes[idx],
             assimpScene->mRootNode->mChildren[idx]) =
        mergeMeshes(meshIter.second, idx);

    assimpScene->mMeshes[idx]->mMaterialIndex = meshIter.first;

    if (meshIter.first < mMaterials.size()) {
      assimpScene->mMeshes[idx]->mName.Set(
          mMaterials[meshIter.first].mName.c_str());
      assimpScene->mRootNode->mChildren[idx]->mName.Set(
          mMaterials[meshIter.first].mName.c_str());
    }
    idx++;
  }
...
void AssimpWriter::buildMegaMesh(aiScene *assimpScene) {
  ...
  std::tie(assimpScene->mMeshes[0], assimpScene->mRootNode->mChildren[0]) =
      mergeMeshes(mMeshes, 0);
}
...
void AssimpWriter::buildMeshPerObject(aiScene *assimpScene) {
  ...
  assimpScene->mRootNode->mChildren =
      new aiNode *[mMeshes.size() + mLocatorNodes.size()];
...
int AssimpWriter::write(const std::filesystem::path &filename) {
  ...
  auto outExt = filename.extension().string();
  outExt = outExt.substr(1); // remove dot

  return mExporter.Export(assimpScene, outExt, filename.string(),
...
void AssimpWriter::addLocator(aiNode *node) { mLocatorNodes.push_back(node); }
