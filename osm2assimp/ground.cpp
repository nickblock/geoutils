...
namespace GeoUtils
{
  ...
  void Ground::addSubtraction(const OSMFeature &feature)
  {
    ...
    while (merged)
    {
      ...
      for (auto bIter = mSubs.begin(); bIter != mSubs.end(); ++bIter)
      {
        ...
        {
          auto result = intersectPolygons(std::get<Poly>(boxPoly), std::get<Poly>(*bIter));
  ...
  stringstream pointsss(const std::vector<glm::vec2> &points, const glm::vec2 &min, float scale)
  {
    stringstream ss;
  ...
  void Ground::writeSvg(const std::string &path, float scale)
  {
    ...
    file << tfm::format("<svg viewBox = \"%d %d %d %d\" xmlns = \"http://www.w3.org/2000/svg\">", 0, 0, (mBBox.mMax.x - mBBox.mMin.x) * scale, (mBBox.mMax.y - mBBox.mMin.y) * scale) << endl;
