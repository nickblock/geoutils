...
rapidjson::Value osmiumBoxToJSON(const osmium::Box& box, rapidjson::Document::AllocatorType& alloc) {
...
void OSMSplitConfig::initFromJSON(const rapidjson::Value& value)
{
  ...
  if(value.HasMember("splitLess")) {
    ...
    if( abs(mExtents.top_right().lat() - mExtents.bottom_left().lat()) > abs(mExtents.top_right().lat() - mExtents.bottom_left().lat())) {
      mSortByLat = true;
    }
    else {
      mSortByLat = false;
...
OSMSplitConfig::OSMSplitConfigPair OSMSplitConfig::split(double midPoint)
{
  ...
  mSplit.first = make_shared<OSMSplitConfig>(lessBox, mFileName + string("0"), !mSortByLat);
  mSplit.second = make_shared<OSMSplitConfig>(moreBox, mFileName + string("1"), !mSortByLat);

  return mSplit;
}


rapidjson::Value OSMSplitConfig::toJson(rapidjson::Document::AllocatorType& allocJS) const
{
  ...
  if(mSplit.first) {
    ...
    configJS.AddMember("fileName", mFileName, allocJS);
  }
...
std::vector<std::string> OSMSplitConfig::filesForBox(const osmium::Box& box) const
{
  ...
  if(box.bottom_left().x() > mExtents.top_right().x() ||
    ...
    if(mSplit.first) {
      std::vector<std::string> firstNames = mSplit.first->filesForBox(box);
      if(firstNames.size()) {
        fileNames.insert(fileNames.end(), firstNames.begin(), firstNames.end());
      }
      std::vector<std::string> secondNames = mSplit.second->filesForBox(box);
      if(secondNames.size()) {
        fileNames.insert(fileNames.end(), secondNames.begin(), secondNames.end());
      }
    }
    else {
...
string OSMSplitConfig::getFileName() const
{
  return mFileName + suffix();
}
}